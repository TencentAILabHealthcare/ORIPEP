...
def main():
    ...
    # Add task argument
    parser.add_argument('--task', type=str, choices=['batch', 'single'], default='single',
    ...
    if args.task == 'batch':
        test_results = exp.predict_batch(checkpoints=checkpoints, save_prediction=True, save_df_name=f'{args.task}.tsv')
    
    elif args.task == 'single':
        exp.predict_single(checkpoints, protein_id='3dab_G', peptide_seq='TSFAEYWNLLSP')
